...
const UserRegister = () => {
  ...
  const addUserToCommunityInBackground = async (communityName) => {
    try {
      ...
      console.error('Failed to add user to community (background):', error);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Background tasks: ensure community exists, then add user to it
      createCommunityInBackground(communityName).then(() => addUserToCommunityInBackground(communityName));
