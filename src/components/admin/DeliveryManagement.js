...
const DeliveryManagement = () => {
    ...
    try {
      ...
      Object.entries(basicTotalsBySpotCustomer).forEach(([spot, byCustomer]) => {
        ...
        Object.entries(byCustomer).forEach(([normalizedName, totals]) => {
          if (totals.totalPrice > 37 || totals.totalQty > 4) {
      ...
      const cratesContentMap = {}; // { [spot]: { [normalizedName]: { displayName, itemsMap: { [key]: {productName, selectedOption, quantity, businessName} } } } }
      validOrders.forEach(order => {
        const pickupSpot = order.customerDetails?.pickupSpot || 'לא צוין';
        const rawCustomerName = order.customerDetails?.name || 'לקוח לא ידוע';
        const normalizedCustomerName = rawCustomerName.trim();
          ...
          Object.values(order.orderBreakdown).forEach(businessOrder => {
            ...
            (businessOrder.items || []).forEach(item => {
              ...
              if (isBasic && isEligible) {
                // add to crate contents for this customer
              ...
              // general list (non-basic, or not eligible)
      ...
      // Convert crates content to arrays for rendering
  ...
  const generatePickupSpotPDF = async (pickupSpot) => {
    try {
      ...
      // Helper to render a page with given HTML content
      ...
      // Build pages: split crates and items into chunks for readability
      const CRATES_PER_PAGE = 2;
      ...
      pdf.save(`משלוחים_${pickupSpot.replace(/\s+/g, '_')}.pdf`);
    } catch (e) {
