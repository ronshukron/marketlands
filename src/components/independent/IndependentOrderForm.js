...
const IndependentOrderForm = () => {
  ...
  const addToCart = (productIndex) => {
    ...
    Swal.fire({ title: 'נוסף לסל!', text: `${product.name} נוסף לסל הקניות שלך`, icon: 'success', timer: 1500, showConfirmButton: false });
  };
        ...
        <div className="bg-white rounded-xl shadow-md overflow-hidden mb-8">
          <div className="p-6">
            ...
            <div className="flex gap-3 mt-2">
              ...
              <Link to="/cart" className="bg-gray-200 hover:bg-gray-300 text-gray-900 py-2 px-3 rounded">מעבר לעגלה</Link>
            </div>
          </div>
        </div>
            ...
            <div key={product.id} className={`bg-white rounded-lg shadow-sm overflow-hidden ${product.stockAmount <= 0 ? 'opacity-60 grayscale' : ''}`}>
              ...
              <div className="p-3 space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => addToCart(index)} disabled={product.stockAmount <= 0} className={`flex-1 ${product.stockAmount > 0 ? 'bg-blue-500 hover:bg-blue-600' : 'bg-gray-400 cursor-not-allowed'} text-white py-1.5 px-3 rounded-md text-sm font-medium flex items-center justify-center gap-1`}>
